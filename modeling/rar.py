...
config_path = "/home/alisonwh/1dTk/configs/training/generator/rar.yaml"
...
class RAR(BaseModel):
    ...
    def generate(self,
        ...
        for step in range(self.image_seq_len):
            # Compute the dynamic guidance scale at each step
            ...
            # Apply cfg_scale to logits for conditional generation
            if guidance_scale != 0:
                logits = logits[:, -1]  # Only use logits for the last token
                cond_logits, uncond_logits = logits[:num_samples], logits[num_samples:]
                logits = uncond_logits + (cond_logits - uncond_logits) * cfg_scale
            else:
                logits = logits[:, -1]  # Just use unconditioned logits if no guidance

            # Dynamic temperature adjustment for each step
            adaptive_temperature = max(0.7, 1.2 - (0.5 * step / self.image_seq_len))
            logits = logits / adaptive_temperature

            # Apply Top-k sampling with dynamic temperature control
            topk_values, topk_indices = torch.topk(logits, 100, dim=-1)
            probs = F.softmax(topk_values, dim=-1)
            sampled = torch.multinomial(probs, num_samples=1)
            sampled_token = torch.gather(topk_indices, -1, sampled)

            # Update the token sequence
            ids = torch.cat((ids, sampled_token), dim=-1)
