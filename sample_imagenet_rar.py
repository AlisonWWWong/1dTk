...
def main():
    ...
    if rank == 0:
        # log: 12/02
        # 手动下载的maskgit-vqgan-imagenet-f16-256.bin文件路径
        tokenizer_path = "./models/maskgit-vqgan-imagenet-f16-256.bin"
        
        # 手动下载的rar生成器权重文件路径
        generator_checkpoint_path = "./models/rar_b.bin"

        # 加载本地文件而非从huggingface下载
        tokenizer = create_pretrained_tokenizer(config, tokenizer_path)
        generator = demo_util.get_rar_generator(config, generator_checkpoint_path)
