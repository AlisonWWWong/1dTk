...
def get_config():
    ...
    print("Using config file:", args.config)  # 打印路径，确保正确
    return OmegaConf.load(args.config)  # 确保传入的是 YAML 文件路径，而不是 Namespace

# from omegaconf import OmegaConf
# import os

# def get_config():
#     # 默认的配置文件路径，可以根据实际情况修改
#     default_config_path = '/home/alisonwh/1dTk/configs/training/generator/maskgit.yaml'

#     try:
#         # 加载配置文件
#         config = OmegaConf.load(default_config_path)
#     except Exception as e:
#         print(f"Error loading config file {default_config_path}: {e}")
#         return None

#     # 设置默认值，确保缺失的字段有容错处理
#     # 使用 `getattr()` 确保在字段缺失时使用默认值
#     config.experiment.output_dir = getattr(config.experiment, 'output_dir', 'default_output')
#     config.experiment.tokenizer_checkpoint = getattr(config.experiment, 'tokenizer_checkpoint', 'default_tokenizer_checkpoint')
#     config.training.seed = getattr(config.training, 'seed', 42)  # 默认随机种子
#     config.optimizer.params.learning_rate = getattr(config.optimizer.params, 'learning_rate', 0.0004)  # 默认学习率
#     config.lr_scheduler.params.warmup_steps = getattr(config.lr_scheduler.params, 'warmup_steps', 62500)  # 默认warmup steps
#     config.model.vq_model.codebook_size = getattr(config.model.vq_model, 'codebook_size', 1024)  # 默认codebook_size
